...
> - **Trang Cài đặt + Đổi mật khẩu** — mục "Cài đặt" trong Navbar dropdown mở `/settings/:section?` (deeplink). Sidebar 7 mục: Bảo mật / Hồ sơ / Giao diện (active) + Thông báo / Riêng tư / Ngôn ngữ / Dữ liệu (placeholder có gợi ý cụ thể). Nhật ký: [ngay2.md § SET-1](./ngay2.md#set-1--cài-đặt--đổi-mật-khẩu-cùng-ngày).
...
- ✅ **Trang hồ sơ** — MVP + Sửa hồ sơ đã làm: avatar (thực hoặc initial), tên, ngày tham gia, số bài, **bio**, lưới bài, **dialog sửa name + bio + picture**. Còn thiếu: upload avatar thật (file storage) — xem [ngay2.md § PROF-5](./ngay2.md#prof-5--sửa-hồ-sơ-name--bio--picture-cùng-ngày).
- ✅ **Trang Cài đặt** `/settings/:section?` — sidebar 7 mục với deeplink: Bảo mật (đổi mật khẩu), Hồ sơ (link), Giao diện (theme), Thông báo / Riêng tư / Ngôn ngữ / Dữ liệu (placeholder có gợi ý). Mở rộng tiếp: SET-2 (Google set initial password), SET-3 (quên mật khẩu — cần email), SET-4 (JWT revoke list — đăng xuất mọi thiết bị).
