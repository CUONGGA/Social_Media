...
| D2-22 | **"Sửa hồ sơ" là stub `alert`** (PROF-5) + **chưa có bio** (PROF-4) + **Navbar avatar/tên không cập nhật sau khi sửa hồ sơ tại trang Profile**. | 🟡 | Endpoint `PATCH /user/:id` owner-only, whitelist name/bio/picture. Dialog form trên client. Sync localStorage + `state.auth.authData` qua dispatch `AUTH` → Navbar (useSelector authData) re-render NGAY. Xem [ngay2.md § PROF-5](./ngay2.md#prof-5--sửa-hồ-sơ-name--bio--picture-cùng-ngày). |
| D2-23 | **Trên Profile, xoá bài → toast hiện nhưng card vẫn nguyên trong grid; like/update cũng không reflect**. Vì `state.profile.posts` là slice riêng còn action `DELETE/UPDATE/LIKE` chỉ áp lên `state.posts.posts`. | 🟠 | `reducers/profile.js` lắng nghe thêm `DELETE` (filter + giảm `viewed.postCount`), `UPDATE` (replace), `LIKE` (merge — tương thích payload partial SSE). Không thêm action type mới. Đóng luôn phần DELETE của PROF-3. Xem [ngay2.md § Hotfix sau PROF-5](./ngay2.md#hotfix-sau-prof-5--profile-grid-không-refresh-sau-khi-deleteupdatelike). |
...
| PROF-2 | 🟡 | **Avatar profile chỉ là chữ cái đầu** — **Đã giải quyết một phần** (Profile header + Navbar hiển thị `picture` từ DB, đổi được qua dialog PROF-5), Post card và Post details vẫn dùng initial. | Mở rộng: populate `picture` của creator vào response của `getposts` / `getPost`. |
| PROF-3 | 🟢 | **`postCount` ở header profile không tự sync** khi **CREATE** (DELETE đã đóng ở D2-23). | DELETE đã sync (giảm 1). CREATE còn lại nhưng ít ưu tiên vì Form chỉ ở Home → user navigate ra rồi quay lại profile thì `getUserProfile` đã fetch số mới. Có thể đóng nốt khi listen `CREATE` trong reducer profile (chỉ áp khi `post.creator === viewed._id`). |
| ~~PROF-4~~ | ✅ | ~~**Chưa có bio / about / social links** trên profile.~~ — **Đã giải quyết** (D2-22). Bio (max 280) hiển thị header. Social links chưa làm. | ✅ Schema `bio` + UI; social links là feature riêng. |
| ~~PROF-5~~ | ✅ | ~~**Nút "Sửa hồ sơ" là stub `alert`**~~ — **Đã giải quyết** (D2-22). | ✅ Dialog edit (`PATCH /user/:id`, owner-only) cho name + bio + picture URL. Xem [ngay2.md § PROF-5](./ngay2.md#prof-5--sửa-hồ-sơ-name--bio--picture-cùng-ngày). |
| PROF-5a | 🟡 | **`post.name` không backfill khi user đổi tên** — Post cũ vẫn hiện tên cũ vì `post.name` denormalized. | Đi cùng D-2 (creator → ObjectId ref User + populate). |
...
| SET-2 | 🟡 | **Google user không thể set initial local password** — đăng nhập Google → muốn có password riêng để login bằng email/password → chưa có flow. | Endpoint `POST /user/me/password/set` (auth + email verify code). UI nhánh thứ 2 trong SettingsDialog cho user Google: "Tạo mật khẩu local". |
| SET-3 | 🟡 | **Quên mật khẩu** — user local quên password thì stuck. Không có "forgot password". | Cần SMTP setup (Mailgun/Sendgrid). Endpoint `POST /user/password/reset-request` (gửi email link 1 lần) + `POST /user/password/reset-confirm`. |
| SET-4 | 🟡 | **JWT cũ vẫn valid sau đổi password** — không có revoke list. Đổi password = chưa "đăng xuất các thiết bị khác". | Redis-backed token blocklist (jti), hoặc rotate JWT_SECRET version theo user. Cần Redis trước. |
...
- **14/05/2026 (cùng ngày, sau PROF-5)** — Đóng PROF-4 + PROF-5 (D2-22): dialog "Sửa hồ sơ" thật với name + bio + picture URL, server PATCH owner-only, sync Navbar realtime. Thêm PROF-5a (post.name không backfill — đi cùng D-2). Còn PROF-3 + PROF-6 + PROF-7.
- **14/05/2026 (cùng ngày, sau hotfix Profile sync)** — D2-23: reducer `profile` lắng nghe `DELETE`/`UPDATE`/`LIKE` để grid + postCount tự cập nhật khi user thao tác trên Profile. Đóng nốt phần DELETE của PROF-3 (hạ severity xuống 🟢 — CREATE còn lại nhưng ít ưu tiên).
- **14/05/2026 (cùng ngày, sau SET-1)** — D2-24: thêm trang **Cài đặt** `/settings/:section?` với sidebar 7 mục (Bảo mật, Hồ sơ, Giao diện active; Thông báo, Riêng tư, Ngôn ngữ, Dữ liệu placeholder + gợi ý). Endpoint `PATCH /user/me/password` (auth-only, bcrypt verify + hash, branch Google). Pivot từ modal sang page trong cùng pass. Xem [ngay2.md § SET-1](./ngay2.md#set-1--cài-đặt--đổi-mật-khẩu-cùng-ngày). Đề xuất follow-up: **SET-2** (Google user set initial password — cần email verify), **SET-3** (Quên mật khẩu — cần email reset), **SET-4** (JWT revoke list — đăng xuất mọi thiết bị).
