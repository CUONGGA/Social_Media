...
| ~~PROF-3..5~~ | ~~Edit form, bio chưa làm.~~ → **PROF-4 + PROF-5 đã làm** ở pass tiếp theo bên dưới ([§ PROF-5](#prof-5--sửa-hồ-sơ-name--bio--picture-cùng-ngày)). PROF-3 (postCount sync khi xoá bài) còn lại. | 🟡 |

---

# PROF-5 — Sửa hồ sơ (name + bio + picture, cùng ngày)

> Tiếp nối ngay sau PROF-1 Phase M. Trước đây nút "Sửa hồ sơ" trong header Profile chỉ là `alert("Tính năng sửa hồ sơ sẽ ra mắt sau.")` — nay thành form thật. Cùng pass đóng PROF-4 (bio) — chỉ vài dòng schema + UI hiển thị.

## Mục tiêu

- Owner có thể đổi **tên hiển thị**, **bio**, **URL ảnh đại diện** ngay tại trang `/users/:id`.
- Update tức thì cả Profile header VÀ Navbar avatar/tên (không phải logout/reload).
- Server chặn cứng: chỉ chính chủ mới sửa được; client KHÔNG cho gửi email/password/googleId.

## Quyết định kỹ thuật

| Tiêu chí | Lựa chọn | Lý do |
|----------|----------|-------|
| Endpoint | `PATCH /user/:id` | RESTful. `auth` middleware có sẵn → server kiểm `req.userId === :id` → 403 nếu khác. |
| Whitelist field | **name, bio, picture** | Email/password/googleId là identity → đổi qua flow riêng (signup/Google OAuth). Tránh leo quyền + tránh phá unique constraint email. |
| `_id` / `creator` / `createdAt` | **Loại khỏi body** | Phòng client gửi để overwrite. Helper inline trong controller — không dùng deep-merge thư viện. |
| Validation | **Inline trong controller** | Pass này nhỏ, không lôi `express-validator` vào (sẽ là pass riêng cho cả app — xem S-6 trong `VanDeCanGiaiQuyet.md`). |
| Giới hạn | name 1-80, bio 0-280, picture URL 0-1024 ký tự | 280 = chuẩn Twitter cũ, đủ cho mô tả ngắn. URL 1024 dư cho Cloudinary/Google. |
| Picture validation | **Chỉ check length**, không regex URL | URL parsing tinh vi (data URI, IPFS, ipv6…) — chấp nhận bất cứ chuỗi nào, browser tự render hoặc gãy. UX edge case → user thấy avatar không hiện thì sửa lại. |
| Response shape | **Đầy đủ giống `GET /user/:id`** | Client dispatch thẳng `FETCH_USER_PROFILE` không phải merge. +1 query nhỏ (`countDocuments`) — chi phí biên vì PATCH hiếm. |
| Sync với Navbar | **Dispatch `AUTH`** sau khi update nếu self → auth reducer ghi `localStorage` + state | Navbar trước đây chỉ re-đọc localStorage trên `location` change → ở trang Profile sửa xong sẽ stale. Subscribe `authData` qua `useSelector` giải quyết. |
| Vùng update Redux | **`state.profile.viewed`** (qua `FETCH_USER_PROFILE`) + **`state.auth.authData`** nếu self | 2 slice tách riêng → 2 dispatch độc lập. Reducer không cần code mới. |
| Form UI | **Dialog modal MUI** (không inline trên trang) | Tiêu chuẩn cho edit ngắn; tránh route mới `/users/:id/edit`; giữ Profile làm "view-only landing". |
| Bio xuống dòng | `white-space: pre-line` ở header | User nhập textarea → giữ ngắt dòng khi hiển thị. |

## Hợp đồng API

### `PATCH /user/:id`

- **Auth**: bắt buộc (Bearer JWT local). Server so sánh `req.userId === :id`.
- **Body** (mọi field optional, nhưng phải có ít nhất 1):
  ```json
  { "name": "string?", "bio": "string?", "picture": "string?" }
  ```
- **Status**:
  - `200 OK` → response body giống `GET /user/:id` (kèm `postCount` mới).
  - `400` → validation lỗi (length, kiểu, không có thay đổi nào).
  - `401` → token thiếu/sai (middleware `auth`).
  - `403` → user khác đang cố sửa.
  - `404` → id không hợp lệ / không tồn tại.
- **Side effect**: User document trong DB cập nhật. Post cũ KHÔNG bị backfill `post.name` (xem § Hạn chế).

### `GET /user/:id` — mở rộng

- Thêm field `bio` (nullable) trong response. Schema vẫn an toàn với client cũ (field thừa = bỏ qua).
...
| File | Thay đổi |
|------|----------|
| `server/models/user.js` | Thêm `bio: { type: String, maxlength: 280 }`. |
| `server/controllers/users.js` | Thêm `updateUserProfile` (~70 dòng). `getUserPublic` trả thêm `bio`. |
| `server/routes/users.js` | `router.patch('/:id', auth, updateUserProfile)`. |

### Client

| File | Thay đổi |
|------|----------|
| `client/src/api/index.js` | Thêm `updateUserProfile(id, payload)`. |
| `client/src/actions/users.js` | Thêm thunk `updateUserProfile` (dispatch `FETCH_USER_PROFILE` + `AUTH` nếu self). Import `notifySuccess`, `AUTH`. |
| `client/src/components/Profile/EditProfileDialog.jsx` *(mới)* | Dialog form với 3 input + preview avatar. |
| `client/src/components/Profile/profile.jsx` | Thay `alert` bằng `setEditOpen(true)`; render `<EditProfileDialog />`; hiển thị `viewed?.bio` dưới meta. |
| `client/src/components/Profile/styles.js` | Thêm class `headerBio` (whitespace-pre-line, word-break). |
| `client/src/components/Navbar/navbar.js` | `useSelector(state => state.auth?.authData)` + thêm `authData` vào `useEffect` deps để re-đọc localStorage khi profile được sửa. |

## Hành vi end-to-end

1. User mở `/users/<myId>` → click "Sửa hồ sơ" → dialog hiện với form pre-fill `viewed.name/bio/picture`.
2. User sửa, click "Lưu":
   - `setSaving(true)`, disable cả 2 button.
   - Thunk `updateUserProfile` → `PATCH /user/:id` (axios interceptor đã gắn `Authorization: Bearer <token>`).
3. Server: validate → `findByIdAndUpdate` → query `postCount` → trả response đầy đủ shape.
4. Thunk:
   - Dispatch `FETCH_USER_PROFILE` với `{...currentViewed, ...data}` → `state.profile.viewed` cập nhật → Profile header re-render.
   - So `stored.result._id === id`: nếu self → dispatch `AUTH` với `{ result: mergedResult, token: stored.token }` → auth reducer ghi `localStorage.profile` + `state.auth.authData`.
   - `notifySuccess`.
   - Trả `true` → dialog đóng.
5. Navbar: `useSelector(authData)` thấy giá trị mới → `useEffect([location, authData])` chạy → `setUser(JSON.parse(localStorage…))` → avatar/tên cập nhật.

## Edge case đã xử lý

- **Picture URL trống**: form gửi `picture: ''` → server set `picture = ''` → response trả `picture: null` (do `user.picture || null`) → header Profile fallback về initial.
- **Tên rỗng/all whitespace**: client disable nút "Lưu" + server trả 400.
- **Bio quá dài**: client `inputProps={{ maxLength: 280 }}` → không gõ được quá 280; server vẫn kiểm 2 lớp.
- **User khác cố `PATCH /user/<id-người-khác>`**: server 403 `{ message: 'You can only edit your own profile' }`.
- **Token expired giữa lúc submit**: server 401 → toast "Please sign in to continue." → dialog mở (user tự đóng và signin lại). Không auto-logout — bug nhỏ chấp nhận được, pass riêng cho refresh token flow.
- **Network fail**: thunk return false → dialog giữ mở, toast lỗi.
- **Mở dialog → đóng → mở lại sau khi đã save lần trước**: `useEffect([open, initial])` pre-fill lại từ `viewed` mới nhất → form sạch.

## Hạn chế đã biết

| ID | Mô tả | Mức độ |
|----|------|--------|
| PROF-5a | **`post.name` không backfill khi đổi tên** — Post cũ vẫn hiện tên cũ ở Home/PostDetails card vì `post.name` là field denormalized lưu lúc tạo. Để fix triệt để: populate `creator.name` thay vì lưu `post.name` (đi cùng D-2 — `creator: ObjectId ref User`). | 🟡 |
| PROF-5b | **Avatar Google bị overwrite** khi user sửa picture trong dialog → lần sign-in Google sau, controller `googleSignIn` chỉ refresh `picture` khi có `picture` mới từ Google. Nếu user xoá picture local → lần signin Google sẽ set lại từ Google. Hành vi này có ý: tôn trọng lựa chọn của user trong app, nhưng cũng tự khôi phục nếu user xoá. Document để tránh ngạc nhiên. | 🟡 |
| PROF-5c | **Không có upload avatar trực tiếp** — chỉ URL. Khi nào làm D-1 (file storage Cloudinary/S3) sẽ có upload thật. | 🟡 |
| PROF-5d | **Bio không markdown** — chỉ plain text + giữ xuống dòng (`pre-line`). Không có link auto-detection. | 🟢 |

## Kiểm thử thủ công

1. **Đăng nhập**, mở `/me` → click "Sửa hồ sơ".
2. **Đổi tên**: gõ tên mới → Lưu → toast "Hồ sơ đã được cập nhật"; header Profile + avatar Navbar đều đổi NGAY (không reload). Mở `localStorage.profile` (DevTools → Application) → `result.name` đã đổi.
3. **Thêm bio**: gõ vài dòng có xuống dòng → Lưu → bio hiện dưới meta, giữ format xuống dòng.
4. **Dán URL ảnh** (vd `https://i.pravatar.cc/300`) → Lưu → avatar header + navbar cập nhật ảnh.
5. **Xoá ảnh**: clear field URL → Lưu → avatar về chữ cái đầu.
6. **Cố sửa user khác**: trong DevTools Console:
   ```js
   fetch('http://localhost:5000/user/<otherId>', {
     method: 'PATCH',
     headers: {
       'Content-Type': 'application/json',
       Authorization: `Bearer ${JSON.parse(localStorage.profile).token}`,
     },
     body: JSON.stringify({ name: 'hacked' }),
   }).then(r => r.json()).then(console.log);
   ```
   Kỳ vọng **403** `{ message: 'You can only edit your own profile' }`.
7. **Tên rỗng**: clear field name → nút "Lưu" disabled. Nếu hack qua DevTools gọi API với name `'   '` → server 400 `Name phải dài 1-80 ký tự`.
8. **Bio quá 280**: gõ paste 300 ký tự → input cắt ở 280 (do `maxLength`). Server cũng kiểm 280.

## Tham chiếu mã

- Server validate: `server/controllers/users.js::updateUserProfile`.
- Owner check: cùng controller, dùng `String(req.userId) !== String(id)` → 403.
- Sync localStorage + auth state: `client/src/actions/users.js::updateUserProfile` (dispatch `AUTH` nếu self).
- Re-render Navbar khi authData đổi: `client/src/components/Navbar/navbar.js` — `useSelector(state => state.auth?.authData)` + dep `useEffect`.

## Mở rộng đề xuất tiếp theo (Phạm vi M phần còn lại)

- ~~**PROF-3 (DELETE)**~~ — **Đã đóng** một phần ở hotfix bên dưới (DELETE đồng bộ ngay trong reducer profile). CREATE còn lại nhưng ít ưu tiên (Form chỉ ở Home → user đã navigate).
- **PROF-5a (post.name backfill)** đi cùng D-2 (creator ObjectId ref) — task lớn, không làm rời.
- **Avatar upload thật** đi cùng D-1 (file storage).

## Hotfix sau PROF-5 — Profile grid không refresh sau khi delete/update/like

> **Triệu chứng**: trên trang `/users/:id`, user xoá 1 bài → toast "Memory deleted" hiện → nhưng card vẫn nguyên trong grid "Kỷ niệm của …". Phải F5 mới biến mất. Tương tự: like trên Profile không đổi số (UI), update bài không reflect.

**Gốc rễ**: Profile dùng `state.profile.posts` (slice riêng — quyết định kiến trúc ở MVP để không lẫn với feed Home). Reducer `posts` xử lý `DELETE`/`UPDATE`/`LIKE` chỉ áp lên `state.posts.posts`/`relatedPosts` — không động vào `state.profile.posts`. Khi user thao tác trên Profile, action vẫn dispatch như cũ → toast hiện vì thunk gọi `notifySuccess`, nhưng UI Profile không re-render.

**Cách fix (đã merge)**: `reducers/profile.js` lắng nghe thêm 3 action chia sẻ:

| Action | Hành vi mới trong profile slice |
|--------|---------------------------------|
| `DELETE` | Filter post khỏi `state.posts`; nếu post **thực sự** thuộc grid hiện tại → giảm `viewed.postCount` về tối thiểu 0. Bỏ qua nếu post không có trong grid (tránh trừ nhầm khi xoá ngoài profile). |
| `UPDATE` | Replace post trong `state.posts` theo `_id`. Hỗ trợ trường hợp user sửa bài qua Home rồi quay lại profile. |
| `LIKE` | **Merge** (giống reducer `posts`) thay vì replace — để hỗ trợ cả payload đầy đủ (REST `actions/posts.js::likePost`) lẫn partial `{ _id, likes }` (SSE `like:update` từ A1). |

**Files chạm**: chỉ 1 file — `client/src/reducers/profile.js`. Không cần action mới, không đụng thunk hay component.

**UX polish kèm theo**: trên Profile, nút "..." (MoreHorizIcon) ở overlay góc trên-phải Post card vốn để mở Form edit qua `setCurrentId` — nhưng Profile truyền `setCurrentId={() => {}}` (no-op vì trang không mount Form) → click không có hiệu lực, dễ gây nhầm lẫn. Đã ẩn bằng prop `hideEdit` mới của `Post` component (default false). Files chạm: `client/src/components/Posts/Post/post.js` (thêm prop), `client/src/components/Profile/profile.jsx` (truyền `hideEdit`). Delete vẫn giữ — user xoá bài của mình từ Profile bình thường.

**Phụ thu**: hotfix này cũng đóng phần lớn **PROF-3** (postCount sync khi DELETE). CREATE còn lại nhưng ít ưu tiên — Form tạo bài chỉ có ở Home, user navigate xong rồi quay lại profile thì `getUserProfile` đã fetch số mới.

**Kiểm thử thủ công**:

1. Mở `/users/<myId>` → grid có vài bài, header hiển thị `N kỷ niệm`.
2. Click "Delete" trên 1 card → toast "Memory deleted" → card **biến mất ngay** + số "kỷ niệm" giảm 1.
3. Click like trên 1 card → số like đổi tức thì (vẫn dùng optimistic).
4. Mở 2 tab cùng `/posts/<bài-thuộc-mình>`: bấm like ở tab A → tab B nếu mở `/users/<myId>` (cùng bài trong grid) cũng thấy số like đổi (qua SSE `like:update` → reducer LIKE → cả 2 slice merge).
5. Sửa 1 bài qua Home Form → nội dung mới: navigate sang profile của mình → bài đã update (UPDATE đã propagate).

**Lý do thiết kế — vì sao không refactor Profile dùng chung `state.posts`**: đã chốt ở MVP rằng tách slice giúp Profile không lẫn với feed Home/search (xem [§ Trang hồ sơ — Phạm vi S](#trang-hồ-sơ--phạm-vi-s-mvp-cùng-ngày), bảng "Quyết định kỹ thuật", row "State client"). Lắng nghe action chia sẻ là chi phí biên (~30 dòng), bù lại giữ ranh giới rõ ràng giữa 2 ngữ cảnh.

## Hotfix nhỏ — Tag không được chứa `,` và khoảng trắng

> **Triệu chứng**: ở Form tạo/sửa bài, user gõ `"summer vacation"` → lưu thành 1 tag `"summer vacation"` (chứa space). Ở Home ChipInput, `"summer vacation"` + Enter cũng tạo 1 chip có khoảng trắng. Yêu cầu: tag phải là 1 từ liền, không chứa `,` (đang là separator hiển nhiên) và không chứa khoảng trắng.

**Giải pháp** (nhẹ nhất — không hiển thị error): coi cả `,` và mọi whitespace là **separator**, tự split ngay tại điểm input. UX giống hashtag Twitter — user không "gõ sai" được, mọi paste đều được normalize.

| File | Thay đổi |
|------|----------|
| `client/src/components/Forms/form.js` | Thêm state riêng `tagsInput` (string thô user gõ) tách khỏi `postData.tags` (array đã parse). TextField hiển thị `tagsInput`, mọi keystroke vừa cập nhật `tagsInput` vừa parse vào `postData.tags` qua `handleTagsChange`. Khi load post để edit → derive `tagsInput` từ `post.tags.join(', ')`. `clear()` reset cả 2. |
| `client/src/components/Home/home.js` | `handleAdd` (cho ChipInput) split chuỗi vào theo `/[,\s]+/` rồi push từng mảnh, **dedupe** bằng `Set` để không thêm tag đã có. Thêm prop `newChipKeys={['Enter', ',']}` để gõ phẩy cũng tạo chip ngay (không thêm `Space` vào trigger keys vì user có thể đang gõ giữa 1 từ dài — sẽ làm gãy UX). |

### Lý do cần state `tagsInput` riêng — bug đã gặp & sửa

Pass đầu tôi giữ TextField là **fully controlled** từ array đã parse:

```js
value={postData.tags.join(', ')}
onChange={(e) => setPostData({ ...postData, tags: e.target.value.split(/[,\s]+/).filter(Boolean) })}
```

Hậu quả: user gõ `vacation` rồi gõ space → `e.target.value === "vacation "` → parse thành `["vacation"]` → re-render với `value === "vacation"` (mất trailing space). Mắt user thấy: **"phím space không có tác dụng"**. Tương tự với `,`. Chữ thường thì OK vì không bị regex bóc.

Fix: tách input state ra. `tagsInput` lưu nguyên văn user gõ (cho display), `postData.tags` lưu kết quả parse (cho submit). Hai state này luôn đồng bộ qua `handleTagsChange` — nhưng cái cho UI là chuỗi thô, nên separator hiển thị bình thường.

> Bài học rút ra: với input mà giá trị nội bộ được normalize (parse → format → parse vòng lặp), **đừng** dùng kết quả format làm `value` của input controlled. Phải lưu raw string riêng. Đây là pattern tương tự khi nhập số có dấu phẩy nghìn (`1,234,567`), nhập ngày, hoặc bất kỳ input nào có quy tắc format hiển thị.

**Kết quả** (cả 2 nơi):

- `"vacation, family"` → `["vacation","family"]`
- `"vacation family"` → `["vacation","family"]`
- `"vacation,family"` → `["vacation","family"]`
- `"  a  ,  b  c "` → `["a","b","c"]`
- Paste `"react, redux node js"` → 4 tag: `react`, `redux`, `node`, `js`.

**Phía server**: schema `post.tags` đang là `[String]` — không cần đổi. Backend nhận được tag đã sạch từ client. Phòng thủ thêm validation server-side có thể làm sau (xác nhận `/^\S+$/` cho mỗi phần tử).

**Files chạm**: 2 file client như bảng trên. Không thunk, không reducer, không API.

---

# SET-1 — Cài đặt & Đổi mật khẩu (cùng ngày)

> Trước đây menu profile chỉ có 3 mục: **Hồ sơ / Theme toggle / Logout**. Không có "Cài đặt" và không có cách nào đổi mật khẩu cho user local. Pass này bổ sung mục **Cài đặt** trong Navbar dropdown → mở **trang `/settings`** chứa sidebar + nhiều section (mỗi section 1 nhóm tuỳ chỉnh). Section "Bảo mật → Đổi mật khẩu" hoạt động đầy đủ, các section khác đã có placeholder + danh sách gợi ý cụ thể để phát triển sau.

> **Lịch sử pass này** — ban đầu làm bằng `SettingsDialog` (modal) vì chỉ có 1 setting. Sau đó user yêu cầu page riêng (có sidebar nhiều mục) để dễ mở rộng. Pivot sang `/settings/:section?` route, xoá `SettingsDialog.jsx`. Bài học: khi feature dự kiến có ≥3 sub-area, page > modal ngay từ đầu — chuyển page sau cũng được nhưng phải refactor.

## Quyết định kỹ thuật

| Câu hỏi | Lựa chọn | Lý do |
|---------|----------|-------|
| Dialog hay trang riêng? | **Trang `/settings/:section?`** | Sidebar nhiều mục — modal không đủ rộng + không có URL → khó share link cụ thể "Bảo mật". Route param cho deeplink chuẩn (Gmail, GitHub đều làm vậy). Default `/settings` → redirect `/settings/security`. |
| Endpoint? | `PATCH /user/me/password` | RESTful, identity LẤY TỪ JWT (không lấy từ body / URL param). `/me/*` namespace dành riêng cho "tôi" — không thể impersonate qua URL. |
| Mật khẩu hiện tại cần? | **Có** | Defense-in-depth: nếu attacker mượn được session/token (XSS, máy không khoá), vẫn cần biết password cũ. Tốn 1 giây nhập, đáng. |
| Tối thiểu length? | **6 ký tự** | Cùng ngưỡng client+server, set ở 1 nơi (`PASSWORD_MIN` ở client, hardcode đồng số ở controller). Có thể nâng 8 + rule mạnh (chữ hoa, số, ký tự) sau. |
| User Google không có password local → ứng xử? | **Banner thông báo + link tới Google account settings** | Server trả `400 { code: 'NO_PASSWORD' }`. Client detect SỚM bằng `localStorage.profile.result.password` (presence) → render banner thay vì form. Server vẫn là nguồn-sự-thật (defense in depth). |
| Đổi password xong có invalidate session khác? | **Không** | JWT signed locally, không có refresh token / blocklist. Implement đúng cần Redis + ID-revoke list — task lớn, hoãn. JWT cũ vẫn valid sau khi đổi password = giới hạn đã biết. |
| Show/hide password? | **Có, riêng từng field** | Bật field "current" xem có gõ đúng không, không lộ field "new" → giảm shoulder-surfing. 3 toggle độc lập. |
| Section "Hồ sơ" trong Settings có form sửa hồ sơ riêng? | **Không** — chỉ deeplink tới `/users/:id` | DRY UX: hồ sơ có 1 chỗ sửa duy nhất (dialog ở Profile page). Settings → "Hồ sơ" chỉ là card preview + nút "Mở trang hồ sơ". Tránh 2 form cùng sửa 1 entity → nhầm lẫn dữ liệu. |
| "Coming soon" sections trông như thế nào? | **Placeholder + danh sách gợi ý cụ thể (4-5 ý mỗi mục)** | Trong các app trưởng thành, settings page rỗng quá → trông như chưa làm. Có gợi ý cụ thể = roadmap mini cho dev tương lai + signal cho user "team đang nghĩ tới". Badge "Sắp ra mắt" rõ ràng để không gây hiểu lầm. |
...
| Mã | Khi nào | Body |
|----|---------|------|
| `200` | Thành công | `{ "message": "Đổi mật khẩu thành công" }` |
| `400` | Mật khẩu mới < 6 ký tự | `{ "message": "..." }` |
| `400` | newPassword !== confirmPassword | `{ "message": "..." }` |
| `400` | newPassword === currentPassword | `{ "message": "..." }` |
| `400` | currentPassword sai | `{ "message": "Mật khẩu hiện tại không đúng" }` |
| `400` | User Google không có password local | `{ "message": "...", "code": "NO_PASSWORD" }` |
| `401` | Thiếu/expired token | `{ "message": "...", "code": "NO_TOKEN \| BAD_TOKEN" }` |
| `404` | User document đã bị xoá | `{ "message": "User not found" }` |
| `500` | Lỗi DB | `{ "message": err.message }` |

Server **không** trả password hash mới. JWT cũ vẫn valid → frontend không phải refresh token.

## Cấu trúc trang Settings

```
client/src/components/Settings/
├── Settings.jsx              ← page chính: sidebar + content + auth guard + routing slug
├── styles.js                 ← shared styles dùng cho cả page + sections
└── sections/
    ├── SecuritySection.jsx   ← form đổi mật khẩu (active) — nhánh Google: banner
    ├── AppearanceSection.jsx ← theme toggle thật + 2 option "Sắp ra mắt"
    ├── ProfileSection.jsx    ← preview card + link deeplink /users/:id
    └── ComingSoonSection.jsx ← placeholder dùng chung (Notifications/Privacy/Language/Data)
```

Mảng `SECTIONS` trong `Settings.jsx` là **single source of truth**: thứ tự sidebar, slug URL, icon, label, status. Thêm section mới = thêm 1 entry vào mảng + (nếu active) thêm `case` trong `renderActive()`.

## Sidebar sections + status

| Slug | Tên | Status | Nội dung |
|------|-----|--------|----------|
| `security` | Bảo mật | ✅ **active** | Form đổi mật khẩu (user local) hoặc banner Google. Là landing mặc định. |
| `profile` | Hồ sơ | 🟡 **partial** | Card preview (avatar + name + email) + nút "Mở trang hồ sơ" → `/users/:id`. Sửa thật ở dialog tại Profile page. |
| `appearance` | Giao diện | ✅ **active** | 2 nút Sáng/Tối (gọi `setMode` của `ThemeModeContext`). 2 option khác có badge "Sắp ra mắt": auto theo OS, cỡ chữ & mật độ. |
| `notifications` | Thông báo | 🟡 **soon** | Gợi ý: chuông realtime (A2), email weekly digest, tắt theo loại, do-not-disturb. |
| `privacy` | Quyền riêng tư | 🟡 **soon** | Gợi ý: mặc định public/private, ai được comment, block user, ẩn email, lịch sử đăng nhập. |
| `language` | Ngôn ngữ & vùng | 🟡 **soon** | Gợi ý: i18n VI/EN, múi giờ, định dạng ngày, tuần bắt đầu thứ Hai/CN. |
| `data` | Dữ liệu & tài khoản | 🟡 **soon** | Gợi ý: export JSON (GDPR), đăng xuất mọi thiết bị (cần SET-4), tạm khoá, xoá vĩnh viễn, liên kết Google/GitHub… |

Danh sách gợi ý không phải tuyên bố cam kết — chúng là **roadmap mini** giúp dev tương lai biết hướng đi và giúp user biết product đang nghĩ tới gì.

## Files chạm

| File | Thay đổi |
|------|----------|
| `server/controllers/users.js` | Thêm `changePassword`: verify req.userId; nếu user.password rỗng → 400 NO_PASSWORD; bcrypt compare; validate; bcrypt.hash(12) → save. |
| `server/routes/users.js` | Thêm `router.patch('/me/password', auth, changePassword)`. Đặt TRƯỚC route `/:id` không bắt buộc (segment count khác) nhưng nhóm theo "/me/*" cho dễ đọc. |
| `client/src/api/index.js` | Thêm `changePassword(payload)` → `API.patch('/user/me/password', payload)`. |
| `client/src/actions/users.js` | Thunk `changePassword`. Trả `true` / `{ code: 'NO_PASSWORD' }` / `false`. Trường hợp NO_PASSWORD KHÔNG hiện toast (đã có banner trong section) — tránh thông tin trùng. |
| `client/src/App.js` | Import `Settings`; thêm route `<Route path="/settings/:section?" component={Settings} />` trước route `/auth`. |
| `client/src/components/Settings/Settings.jsx` *(mới)* | Page chính: sidebar 2 cột desktop, stack mobile. Auth guard: chưa login → `<Redirect to="/auth" />`. Slug không hợp lệ → redirect `/settings/security`. Sidebar item click → `history.push('/settings/<slug>')`. Hỗ trợ Enter/Space cho a11y. |
| `client/src/components/Settings/styles.js` *(mới)* | Shared styles: sidebar (sticky, 260px, mobile horizontal scroll), content paper, sectionHeader (icon box 44px), formStack, banner Google, optionRow (Appearance), suggestionList (Coming soon), badge "Sắp ra mắt". Hover shadow override để không lift khi hover. |
| `client/src/components/Settings/sections/SecuritySection.jsx` *(mới)* | Form đổi mật khẩu (logic giống dialog cũ): 3 field show/hide riêng, validation inline (≥6, !== current, === confirm), submit + reset form khi success, banner Google khi `!hasLocalPassword`. |
| `client/src/components/Settings/sections/AppearanceSection.jsx` *(mới)* | 2 nút Sáng/Tối — nút "active" `contained`, nút khác `outlined`. Gọi `useThemeMode().setMode('light'\|'dark')` (set tuyệt đối, không toggle, để click vào nút đang active không "tắt"). Thêm 2 row option "Sắp ra mắt" (auto OS, cỡ chữ). |
| `client/src/components/Settings/sections/ProfileSection.jsx` *(mới)* | Card preview + nút `<Link to="/users/:id">`. Không có form sửa hồ sơ (đã có ở Profile page). |
| `client/src/components/Settings/sections/ComingSoonSection.jsx` *(mới)* | Component dùng chung cho 4 section chưa làm. Props: icon, title, description, suggestions[]. Render badge + danh sách `<ul>` các gợi ý. |
| `client/src/components/Navbar/navbar.js` | Đổi `openSettings()` thành `history.push('/settings')`, xoá state `settingsOpen` + import `SettingsDialog`. MenuItem "Cài đặt" giữ nguyên position + mô tả phụ. |
| `client/src/components/Navbar/SettingsDialog.jsx` | **🗑️ Xoá** — thay bằng trang Settings. Logic password đã chuyển sang `SecuritySection.jsx`. |

## UX & a11y chi tiết

- **Sidebar sticky** desktop (`position: sticky`) → scroll content area mà sidebar vẫn dính. Mobile thì stack ngang scroll-x.
- **a11y sidebar item**: `role="button"` + `tabIndex={0}` + `onKeyDown` (Enter/Space) → keyboard-navigable. Không dùng `<button>` vì cần `<a>`-like với active state phức tạp.
- **Deeplink theo URL slug**: copy link `/settings/privacy` rồi share → bạn bè mở thẳng vào tab Privacy. F5 không reset section.
- **Slug invalid** (vd `/settings/foo`) → `<Redirect />` về `/settings/security`. Không có trang trắng, không có 404 confusion.
- **Reset form sau success**: SecuritySection clear `current`/`next`/`confirm`/`show*` sau khi đổi mật khẩu thành công. User mở lại settings không thấy password cũ vẫn còn trong input.
- **Validation inline có-điều-kiện**: chỉ hiện error khi field có giá trị. Không "scream" lỗi ngay khi mở. Helper text "Tối thiểu 6 ký tự" hiện sẵn ở field new, nhưng đỏ chỉ khi user gõ < 6.
- **Banner Google**: dùng `<a target="_blank" rel="noreferrer">` → tránh tabnabbing.
- **Show password icon size="small"** — không lấn TextField height, vẫn dễ click trên mobile.
- **Appearance: setMode tuyệt đối** thay vì `toggleMode`: click vào nút "Sáng" khi đang ở Sáng = no-op (đúng kỳ vọng). Click vào nút "Tối" = chuyển. Toggle sẽ "tắt" theme khi click nút active → confusing.
- **Sidebar badge "Sắp ra mắt"** ẩn ở mobile (`display: none` < sm) — màn hẹp, label đủ thông tin rồi, badge thừa.

## Edge cases đã xử

| Case | Hành vi |
|------|---------|
| User local có cả password + googleId (signup local rồi link Google) | `result.password` tồn tại → cho phép đổi password local bình thường. Google login vẫn dùng được song song. |
| User Google đổi local password → server vẫn 400 NO_PASSWORD | Client detect sớm qua `hasLocalPassword`. Nhưng nếu localStorage stale (vd user mở 2 tab), server-side 400 sẽ kích hoạt `noPasswordOverride` → switch UI sang banner. |
| Token hết hạn giữa lúc nhập form | Submit → server 401 → axios trả lỗi → toast "Please sign in to continue." (từ `axiosErrorMessage` helper). User về `/auth`. |
| `currentPassword === newPassword` | Validate cả 2 nơi: client (disable submit + không có hint inline, đơn giản nhất), server (400 với message rõ). Phòng client bị bypass. |
| Network drop giữa submit | Axios reject → `submitting` reset → toast lỗi → form giữ giá trị (user không phải gõ lại). |

## Giới hạn đã biết

1. **JWT cũ vẫn valid sau khi đổi password** — đã giải thích phía trên. Mitigation: implement revoke list (R-future).
2. **Không có rate limit** trên endpoint password. Brute force trên `currentPassword` về lý thuyết khả thi nếu attacker đã có JWT. Mitigation đi cùng S-4 (rate limit toàn server).
3. **Tài khoản Google muốn chuyển sang có password local** — chưa hỗ trợ. Phải implement luồng "set initial password" (yêu cầu xác thực email lại) — task riêng (SET-2 đề xuất).
4. **Quên mật khẩu** — chưa có (cần email reset link, smtp setup). Là SET-3 đề xuất.

## Kiểm thử thủ công

1. Đăng nhập **local** → click avatar → menu "Cài đặt" → trang `/settings/security` mở với sidebar 7 mục + content "Bảo mật".
2. Sidebar: click "Giao diện" → URL đổi thành `/settings/appearance`, content đổi → click nút "Tối"/"Sáng" → theme đổi ngay → F5 → vẫn ở `/settings/appearance` với theme đã chọn.
3. Click "Thông báo" → hiện placeholder + danh sách gợi ý + badge "Sắp ra mắt".
4. Click "Hồ sơ" → card preview avatar + email + nút "Mở trang hồ sơ" → click → navigate sang `/users/:id`.
5. Quay lại "Bảo mật" → gõ 3 field hợp lệ → "Đổi mật khẩu" → toast success → form tự clear → đăng xuất → login lại bằng password MỚI: OK.
6. Gõ password mới < 6 → error inline đỏ. Đổi confirm khác new → error confirm. Đổi new === current → submit disabled (không hiện inline để giảm noise).
7. Đăng nhập **Google** → menu "Cài đặt" → trang Settings → "Bảo mật" hiện banner "đăng nhập Google" + link Google Account → click → mở tab mới tới Google.
8. Truy cập trực tiếp `/settings/privacy` (paste URL, F5) → mở thẳng tab Privacy. ✅ deeplink hoạt động.
9. Truy cập `/settings/foo` (slug không có) → tự redirect về `/settings/security`. Không có trang trắng.
10. Chưa đăng nhập, truy cập `/settings` → redirect `/auth`. ✅ auth guard.
11. Resize browser < 600px → sidebar stack lên trên, scroll ngang. Mỗi item vẫn click được, badge "Sắp ra mắt" ẩn để gọn.

## Vì sao section riêng cho Settings (không gộp vào EditProfileDialog)?

- **Phạm vi khác**: profile là "thông tin công khai" (name, bio, avatar) — settings là "tuỳ chỉnh tài khoản" (password, theme, notifications, privacy, language…). Trộn = giao diện rối, user không biết tìm đâu.
- **Phân quyền khác**: profile update có owner check theo `:id` (PATCH /:id). Password update LUÔN cho `req.userId` (PATCH /me/password) — không bao giờ có id trong URL. Logic API riêng → UI riêng cho clean.
- **Tương lai**: cấu trúc page + section pattern đã sẵn để thêm feature. Chỉ cần (a) thêm 1 entry vào `SECTIONS`, (b) tạo file section mới hoặc dùng `ComingSoonSection`. Không cần đụng routing hay layout.
